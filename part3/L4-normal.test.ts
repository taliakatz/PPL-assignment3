...
describe('L4 Normal Eval', () => {
    ...
    it('evaluates the examples', () => {
        // Preserve bound variables in subst //crush
        expect(bind(parseL4(`
            (L4 (define nf
                  (lambda (f n)
                    (if (= n 0)
    ...
    // Part 3 tests
    it('our own test1 - evaluates programs which would give a semantic error in the applicative order, but not in normal order', () => {
        expect(bind(parseL4(`
        (L4 (define z (-))
            (define f (lambda (x) (if (= x 1) (* x x) (* z z))))
                (f 1))`), evalNormalProgram)).to.deep.equal(makeOk(1));
    });

    it('our own test2 - evaluates programs which would give a semantic error in the applicative order, but not in normal order', () => {
        expect(bind(parseL4(`
        (L4 (define f (lambda (x) (if (= x 1) (+ x x) (- 'a x))))
            (f 1))`), evalNormalProgram)).to.deep.equal(makeOk(2));
    });

    it('our own test3 - evaluates programs which would give an error in the applicative order, but not in normal order', () => {
        expect(bind(parseL4(`
            (L4 (let ((a 0))
                (let ((b (/ a a)))
                    (/ a 1))))`), evalNormalProgram)).to.deep.equal(makeOk(0));
    });
});